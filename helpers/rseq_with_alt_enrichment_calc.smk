...
rule compile_annotations:
    input: genome_home_dir + "/{genome}/homer_anno.txt"
    output:
        annot=genome_home_dir + "/{genome}/{genome}_annos/N.bed",
        chromsizes=genome_home_dir + "/{genome}/chrom.sizes"
    params:
        helpers_dir=helpers_dir
    shell: "RScript {params.helpers_dir} parse_annotations.R {wildcards.genome} {input}"


rule sort_bed:
    input:
        macs="{outdir}/peaks_macs_unstranded/{sample}_{genome}_peaks.broadPeak",
        chrom_size=genome_home_dir + "/{genome}/chrom.sizes"
    output: "{outdir}/peaks_macs_unstranded/{sample}_{genome}_peaks.sorted.broadPeak"
    shell: "bedtools sort -i {input.macs} -g {input.chrom_size} > {output}"

rule calculate_annotation_enrichment:
    # Calculates the percentage of called peaks overlapping with repeat regions
    input:
        annots=expand(genome_home_dir + "/{genome}/{genome}_annos/{classes}.bed",
                   genome=[genome,], classes=['N', 'Simple_repeat', 'Satellite', 'P', 'pseudo', 'I', 'TTS', 'LINE',
                                              'LTR', 'SINE', 'DNA', 'CpG-Island', 'ncRNA', 'Low_complexity', 'E',
                                              'snRNA', 'Retroposon', '5UTR', '3UTR', 'srpRNA', 'rRNA', 'tRNA', 'RC',
                                              'scRNA', 'miRNA', 'RNA', 'snoRNA']),
        peaks_macs2="{outdir}/peaks_macs_unstranded/{sample}_{genome}_peaks.sorted.broadPeak",
        chrom_size=genome_home_dir + "/{genome}/chrom.sizes"
    output:
        stats_out="{outdir}/QC/{sample}_{genome}.feature_overlaps.txt"
    shell: """
        echo "Fisher stats" > {output.stats_out}
        for annot in {input.annots}; do
            baseanno=$(basename $annot .bed)
            echo $baseanno >> {output.stats_out}
            bedtools fisher -a {input.peaks_macs2} -b $annot -g {input.chrom_size} >> {output.stats_out}
        done
    """
