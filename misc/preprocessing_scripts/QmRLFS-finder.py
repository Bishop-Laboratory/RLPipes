...
def output_rlfs(riz, rez, seq, strand, desc, model):
    ...
    if args.coordinate_name or args.coordinate:
        ...
        if end_i < start_i:
            ...
            exit(
                "In --coordinate chr:start-end and --coordinate_name, end-start+1 ({}-{}+1={}) must equal to input sequence length ({})\nexclude --coordinate_base_zero option may solve the problem.".format(
                    end_i, start_i, end_i - start_i + 1, seqlength))
        elif seqlength != end_i - start_i + 1:
            exit(
                "\nIn --coordinate chr:start-end and --coordinate_name, end-start+1 ({}-{}+1={}) must equal to input sequence length ({})\ninclude --coordinate_base_zero option may solve the problem.".format(
                    end_i, start_i, end_i - start_i + 1, seqlength))
        start_i = start_i - 1
    else:
        chrom_i = desc
    ...
    if args.coordinate_name or args.coordinate:
        ...
        fasta_comment = "{}.{} ({}:{}-{}) (model={};strand={})".format(name, riz['model'], chrom_i, start + 1, end,
                                                                       model, strand)
    else:
    ...
    if args.fa:
        fa_records.append(
            SeqRecord(Seq(seq[min(riz_rez_sorted):max(riz_rez_sorted)]), id=fasta_comment, description=""))
    if args.bed:
        name = name.replace(" ", ".")
        chrom_i = chrom_i.replace(" ", ".")
        outfile_bed.write("{}\n".format("\t".join(map(str,
                                                      [chrom_i, start, end, "{}.{}".format(name, riz['model']), score,
                                                       strand, start, end, itemRgb, blockCount, blockSize,
                                                       blockStarts]))))
    if args.custom_track:
        name = name.replace(" ", ".")
        chrom_i = chrom_i.replace(" ", ".")
        outfile_custom_track.write("{}\n".format("\t".join(map(str,
                                                               [chrom_i, start, end, "{}.{}".format(name, riz['model']),
                                                                score, strand, start, end, itemRgb, blockCount,
                                                                blockSize, blockStarts]))))
        ...
        for riz in riz_list:
            ...
            if rez:
                ...
                if head:
                    ...
                    if args.custom_track:
                        outfile_custom_track.write(
                            'track name="{}" description="{}" visibility=4 colorByStrand="5,190,255 244,0,224"\n'.format(
